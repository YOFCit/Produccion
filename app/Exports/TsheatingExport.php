...
class TsheatingExport implements FromArray, WithHeadings, WithStyles
{
  protected $equipos = ['SH-01', 'SH-02', 'SH-03', 'SH-04', 'SH-05'];
      ...
      foreach ($this->turnos as $turno) {
        foreach ($this->tipos as $tipo) {
          ...
          foreach ($this->diasSemana as $diaNombre) {
            ...
            $registro = Tsheating::where('equipo', $equipo)
              ->where('turno', $turno)
              ->where('tipo', $tipo)
              ->whereDate('dia', $dia)
              ->first();
