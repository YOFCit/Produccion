...
class TscoatingExport implements FromArray, WithHeadings, WithStyles
{
  protected $equipos = ['SC-01', 'SC-02', 'SC-03'];
      ...
      foreach ($this->turnos as $turno) {
        foreach ($this->tipos as $tipo) {
          ...
          foreach ($this->diasSemana as $diaNombre) {
            ...
            $registro = Tscoating::where('equipo', $equipo)
              ->where('turno', $turno)
              ->where('tipo', $tipo)
              ->whereDate('dia', $dia)
              ->first();
  ...
  public function styles(Worksheet $sheet)
  {
    ...
    // Colorear filas por equipo y tipo
